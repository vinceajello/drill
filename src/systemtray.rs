...
pub struct LinuxTrayHandle {
    sender: std::sync::mpsc::Sender<LinuxTrayMsg>,
}
...
pub enum LinuxTrayMsg {
    UpdateMenu {
        ...
        resp: std::sync::mpsc::Sender<Result<TrayMenuIds, String>>,
    },
}

#[cfg(target_os = "linux")]
impl LinuxTrayHandle {
    pub fn update_menu(&self, tunnels: &Vec<Tunnel>, tunnel_statuses: &[(String, TunnelStatus)]) -> Result<TrayMenuIds, Box<dyn std::error::Error>> {
        let (resp_tx, resp_rx) = std::sync::mpsc::channel();
        self.sender.send(LinuxTrayMsg::UpdateMenu {
            tunnels: tunnels.clone(),
            tunnel_statuses: tunnel_statuses.to_vec(),
            resp: resp_tx,
        }).map_err(|e| e.to_string())?;
        
        let ids = resp_rx.recv().map_err(|e| e.to_string())??;
        Ok(ids)
    }
}
...
pub fn spawn_linux_tray(
    ...
    let (msg_tx, msg_rx) = std::sync::mpsc::channel::<LinuxTrayMsg>();
    let (init_tx, init_rx) = std::sync::mpsc::channel::<Result<TrayMenuIds, String>>();
    ...
    Ok((LinuxTrayHandle { sender: msg_tx }, menu_ids))
}
